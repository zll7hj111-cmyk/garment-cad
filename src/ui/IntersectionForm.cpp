    ...
    : QWidget(parent)
{
    ...
    m_editAngle->setToolTip(cad::ui::TooltipFormatter::action(
        QStringLiteral("射线角度 (°)"),
        QStringLiteral("默认相对目标线段 start→end 方向，90° = 垂直，逆时针为正，支持公式")));
    ...
    m_chkWorldAngle->setToolTip(cad::ui::TooltipFormatter::action(
        QStringLiteral("按绝对角度输入"),
        QStringLiteral("勾选后角度框按绝对世界角度显示/输入，保存时自动反算为相对线段角度")));
    ...
    m_btnClearAim->setToolTip(cad::ui::TooltipFormatter::action(
        QStringLiteral("清除指向点"),
        QStringLiteral("取消对特定端点的直线指向约束，回退至数值/公式角度模式")));  // 取消指向，回退到角度模式
