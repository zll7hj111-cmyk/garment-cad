...
void SegmentConnectionCard::buildConnRow(QVBoxLayout* lay)
{
    ...
    m_btnAngleOnly = new ElaPushButton(QString::fromUtf8("拆开"), m_connRow);
    tuneButton(m_btnAngleOnly);
    m_btnAngleOnly->setToolTip(QString::fromUtf8(
        "快速拆开：保持角度跟随，解除位置吸附（快拆）"));
    m_btnAngleOnly->setVisible(false);
    pointRow->addWidget(m_btnAngleOnly);
    m_btnClearConn = new ElaPushButton(QString::fromUtf8("清除"), m_connRow);
    tuneButton(m_btnClearConn);
    m_btnClearConn->setToolTip(QString::fromUtf8(
        "拆除连接（删除附着），线段恢复为自由状态"));
    m_btnClearConn->setVisible(false);
    pointRow->addWidget(m_btnClearConn);
    pointRow->addStretch();
...
{
    // 连接线段复选框 (唯一连接开关) + 连接保护 (隐藏) + 独立角度.
    m_chkFollowHost = new ElaCheckBox(this);
    m_chkLockConn = new ElaCheckBox(QString::fromUtf8("连接保护"), this);
    m_chkLockConn->setToolTip(QString::fromUtf8(
        "新建连接默认勾选：拖动任一端时整个对一起移动；取消 = 解除焊接（连接保持，拖跟随线即可拆散，拆散走 D 键快拆）。"));
