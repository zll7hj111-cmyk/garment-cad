    ...
    if (dangling != m_danglingStyled) {
        ...
        if (dangling) {
            ...
            m_valueLabel->setStyleSheet(
                ...
                "font-size: 12px; font-weight: bold; color: #8E44AD; background: transparent;");
...
void AngleMeasureCard::paintEvent(QPaintEvent*)
{
    QStyleOption opt;
    opt.initFrom(this);
    QPainter p(this);
                               ...
                               const QString& sourceLabel, bool alternate)
{
    ...
    const QString bg = alternate ? QStringLiteral("#F7F3FB") : QStringLiteral("#FFFFFF");
    setStyleSheet(QStringLiteral(
        "QWidget#AngleMeasureCard {"
        "  background-color: %1;"
        "  border: 1px solid #E0E4E8;"
        "  border-radius: 6px;"
        "}"
        "QWidget#AngleMeasureCard:hover { border: 1px solid #D2B4DE; }"
    ).arg(bg));
    ...
    m_indexLabel->setStyleSheet(
        "font-size: 11px; color: #8E44AD; background: transparent;");
    ...
    m_commentEdit = new QLineEdit(am.comment, m_detail);
