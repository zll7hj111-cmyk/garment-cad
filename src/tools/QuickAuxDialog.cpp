    ...
    , m_prefillT(pt.interpPercent)
{
    ...
    auto* lblId = new QLabel(cad::param::Serial::toHtml(m_pt.serial), this);
    lblId->setTextFormat(Qt::RichText);
    lblId->setStyleSheet("color:#999; font-size:11px;");
    ...
    auto* hint = new QLabel(
        QString::fromUtf8("\u4f4d\u7f6e = \u7ebf\u6bb5\u957f\u00d7\u767e\u5206\u6bd4 + \u5e38\u91cf\uff1b\u53ef\u518d\u8bbe\u504f\u79fb\u89d2\u5ea6/\u504f\u79fb\u8ddd\u79bb\uff08\u89d2\u5ea6\u4ee5\u7ebf\u6bb5\u65b9\u5411\u4e3a 0\u00b0\uff0c\u9006\u65f6\u9488\u4e3a\u6b63\uff09"),  // 位置 = 线段长×百分比 + 常量；可再设偏移角度/偏移距离（角度以线段方向为 0°，逆时针为正）
        this);
