...
void TestSelectWKey::connectionCardNewSemantics()
{
    ...
    cad::tools::SegmentConnectionCard card(&doc, &scene);
    ...
    QCheckBox* chkHost = findChk(QString::fromUtf8("\u4f4d\u7f6e\u5438\u9644"));   // 位置吸附
    QCheckBox* chkLock = findChk(QString::fromUtf8("\u8fde\u63a5\u4fdd\u62a4"));   // 连接保护
    QVERIFY(chkHost);
    QVERIFY(chkLock);
    // 初始: 完整连接且已焊 → 位置吸附 ✓ / 拖动保护 ✓.
    ...
    // 统一状态模型 (用户 2026-12 拍板): 「跟随」严格 = 真的在跟.
    QComboBox* modeCombo = nullptr;
    for (auto* cb : card.findChildren<QComboBox*>())
        if (cb->count() == 2 && cb->itemText(0) == QString::fromUtf8("跟随")
            && cb->itemText(1) == QString::fromUtf8("独立线段")) {
            modeCombo = cb;
            break;
        }
    QVERIFY(modeCombo);
    auto modeEnabled = [&](int idx) {
        const auto* m = qobject_cast<QStandardItemModel*>(modeCombo->model());
        return m && m->item(idx) && m->item(idx)->isEnabled();
    };
    ...
    // 4) 取消「位置吸附」 = 彻底断开 (attachment 删除; 旧实现只变仅角度).
    chkHost->setChecked(false);
    QVERIFY2(doc.attachments().empty(),
             "取消位置吸附 = 彻底断开");
    // 统一模型: 无连接 → 下拉恒「独立线段」; 「跟随」无缓存不可选;
    // 自由视图连接行显示「位置吸附:」形态.
    QCOMPARE(modeCombo->currentIndex(), 1);
    QVERIFY2(!modeEnabled(0), "显式断开后「跟随」不可选 (无缓存可恢复)");
    QVERIFY2(hasRowLabel(QString::fromUtf8("位置吸附:")),
             "自由视图连接行应为「位置吸附:」");

    // 5) 再勾选「位置吸附」 = 记忆宿主一键恢复: attachment 重建、目标不变、
    //    世界朝向反算保持 (零跳变)、默认焊接.
    ...
    QVERIFY2(modeEnabled(0), "连接后「跟随」恢复可选");
    ...
    // 7) 模式切换缓存 (用户 2026-12): 跟随 → 独立线段 → 跟随, 完整连接配置
    //    原样恢复 (宿主/角度/焊接), 且零跳变。切到「独立线段」= 快照+拆除,
    //    缓存存在使「跟随」条目可选 (恢复入口).
    card.setTarget(line.blockId, line.segId);
    QVERIFY(modeCombo);
    QVERIFY2(modeEnabled(0), "已连接 → 「跟随」可选");
    ...
    modeCombo->setCurrentIndex(1);   // 独立线段 → 连接拆除 (配置进缓存)
    QVERIFY(doc.attachments().empty());
    QVERIFY2(modeEnabled(0), "缓存存在 → 「跟随」条目可选 (可恢复)");
    modeCombo->setCurrentIndex(0);   // 跟随 → 缓存恢复
    QCOMPARE(doc.attachments().size(), size_t(1));
    {
        const auto& a = doc.attachments().front();
        QCOMPARE(a.toBlockId, leader.blockId);
        ...
        QVERIFY2(a.isLocked, "缓存恢复必须原样保持焊接");
        QVERIFY2(!a.angleOnly, "缓存恢复不得退化为仅角度");
        const auto* blk = doc.findBlock(line.blockId);
        ...
        if (dAng > M_PI) dAng = 2.0 * M_PI - dAng;
        QVERIFY2(dAng < 1e-9,
                 qPrintable(QStringLiteral("模式切换缓存恢复后方向跳变 %1 度")
                            .arg(dAng * 180.0 / M_PI, 0, 'f', 6)));
    }
}
...
void TestSelectWKey::doubleClickCrossingPrefersActiveLayer()
{
    ...
    for (const Scenario& sc : scenarios) {
        ...
        const auto dialogs = view.findChildren<cad::tools::LinePropertyDialog*>();
