...
### doc_archive_20260829/ —— 架构评审原始档案（2026-12 自 build/ 迁入）

`ARCHITECTURE_REVIEW.md` = 2026-08 全量架构评审原始报告（`ARCHIVE.md` 的底稿）；`P2_HANDOFF_PROMPTS.md` = P2 剩余任务分批交接指令。
结论口径以 `ARCHIVE.md` 为准，本两件保留当时的原始措辞与任务书原文。
原位置 `build/doc_archive_20260829/`（构建目录化石）。
...
### 一致性 / 重复实现审计全案 —— AUDIT_consistency_duplication.md 等（2026-09 生成，2026-12 清账）

全项目「属性一致性 / 重复实现」审计：跨模块 P0×4 + 模块内 P0×36、P1 8+、P2 重复行/块统计；当时 7 守卫全绿但均未捕获——是守卫覆盖盲区的诊断样本。
`AUDIT_consistency_duplication.md` 为正式版（原 `docs/` 根）；`audit_dryrun.md` / `audit_backup_before_78.md` 为中间稿与核销前备份；`ledger_7.8_intro/_ui/_par/_can/_tool.md` 为五模块逐条核销台账。
已落地条目（P1-4/P1-5/P1-6 角度常量、单位、文案收口等）已吸收进 `AGENTS.md` 关键约束区与 `CONVENTIONS.md`；未闭环项以源码现状为准。
原位置 `docs/` 与 `build/`；`gitscratch.md`（0 字节空文件，历史暂存占位）随批迁入，无内容。
...
文内 ctest 计数与 `test_*.cpp` 名称为拆分前状态（现行以 `ctest -N` 为准，2026-12 为 64 用例）。
