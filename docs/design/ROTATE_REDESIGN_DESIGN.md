...
> **2026-09 后续**：用户六点反馈 → 拍板 D1-D5 → 一次性落地 S1-S5（gizmo 三元素语义、任意枢轴刚体化、
> 角度显示单入口），见 `docs/design/ROTATE_ANGLE_UNIFY_DESIGN.md` §8 与 `TROUBLESHOOTING.md`
> 「旋转工具 / 角度模块统一 S1-S5」；**该文现行口径优先于本文档 §2.5/§2.6 的角度与影子描述**
> （影子偏转字段已随「影子偏转 + 冻结角度基准机制已删除」条目下线，§2.6 仅供考古）。
