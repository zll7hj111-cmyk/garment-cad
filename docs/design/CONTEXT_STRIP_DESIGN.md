...
> **2026-12 架构拆分说明（FILE_SPLIT_PLAN_V3 T1.2）**：
> 为治理基线膨胀与 bool 隐式状态爆炸，`ContextStrip.cpp`（原 1665 行）已拆分为五大模块：
> 1. `PlacedPointStripBar.h/.cpp`：放置点专属控件与交互生命周期独立抽象为子控件；
> 2. `ContextStripSessions.cpp`：连接角度与旋转锚心寄生会话实现；
> 3. `ContextStripDisplay.cpp`：只读回填、单位折叠换算与样式渲染；
> 4. `ContextStripEdit.cpp`：写路径命令提交与快照；
> 5. `ContextStrip.cpp`：核心三态焦点状态机与子控件聚合。
...
### 2.3 字段集（一期 + 2026-xx 紧凑化与连接维度按钮）

```
[L12] 名称[____] 长度[__] cm  角度[__] [°|⌒] 连接:拆开 基准:拆开 换向 P3→P1  跟随 L5   Esc 解除锁定
```

| 控件 | 宽度×高 | 行为 |
|---|---|---|
| 编号徽章 | 自适应 ×30 | `Serial::tag(seg->serial)`，等宽 |
| 名称 | 120×30 | 立即应用（无 debounce） |
| 长度 | 80×30 | 200ms debounce + Enter/失焦立即应用 |
| 角度 | 70×30 | 同上；桥线段只读 |
| 单位切换 | 2×32×30 分段 | ° ↔ ⌒（写 `attachment.rotationMode`）；自由线无 attachment 时禁用 |
| 连接: | 标签 + 46×30 | **2026-xx 新增**：位置维度双面开关（与属性对话框「连接」按钮同语义）——标签在外（mutedText 同款，与 名称:/长度(cm): 一致），按钮只显示 拆开/重连 两字面（11px 紧凑字体）；拆开 = angleOnly（位置自由、角度仍跟随），重连 = 位置回原宿主+重新焊接；两维独立，不碰角度维度 |
| 基准: | 标签 + 46×30 | **2026-xx 新增**：角度维度双面开关（与属性对话框「基准」按钮同语义）——拆开 = angleIndependent（独立角），重连 = 恢复角度跟随（反算零跳变）；不碰位置维度；双拆开 = 自由线 |
| 换向 | 46×30 | `ReverseSegmentCommand`，用 `canReverse()` 预判置灰 + 中文原因 tooltip；**旋转会话内转义为「切换锚心」**（2026-12，转发给 ToolRotate，不 push 命令） |
| 基准读数 | 88×30 只读 | `P# → P#`（起点→终点，显示真实串号 tag，2026-12 起与属性对话框同口径，换向后翻转；旋转会话内锚心端在前） |
| 状态徽标 | 自适应 | 跟随 L# / 自由 / 曲线 / 桥线 / 独立角 / 仅角度 —— 一眼看出这条线受谁驱动 |
| 右端提示 | 自适应 | Esc 解除锁定 · Enter 确认 |

**2026-xx 紧凑化**（用户拍板：条带控件偏大）：行高 35→30、标签 12→11、输入定宽
150/110/100→120/80/70、单位/换向/基准按钮同步收窄、间距 6→4，为两个连接维度
按钮腾位。**2026-xx 二轮修整**（用户反馈：长文本按钮 + Ela 默认字号溢出）：
连接/基准 按钮改为「标签在外 + 两字面」式样，条带全部按钮统一 11px 字体。
**连接/基准 双面按钮可用条件** = Pinned + 有附件 + 非连接会话/旋转会话（会话内
不得突变附件）。坐标、图层、延长量、张力仍留给右侧属性面板（条带超过 7 控件
窄窗口折叠的旧警告即此代价，紧凑化后仍可用）。
